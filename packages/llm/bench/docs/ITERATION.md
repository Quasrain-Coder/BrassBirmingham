...
- **关键修正（推翻"模型不会玩"）**：v6-ab 里 base 模型打出一局 **61 VP 的教科书级胜利**——r1 铺运河连 oxford 商人 → 建煤/制造 → **卖出翻面** → 贷款补现金 → 再建再卖。模型**会玩**，问题是**高方差、易漂移**：同一模型其他局却开局建酿酒厂（rank288）、疯狂 pass。它不是不会，是不稳定。
- **超时是隐性杀手**：默认 8s 超时，慢网关下 15% 决策超时→降级启发式（v5-ab 实测）。加 `BRASS_AI_TIMEOUT_MS` 覆盖调到 30s 后 degraded 降到 7%。**之前不少实验的"模型乱选"其实是超时降级在作祟。**
- **v5（编号即价值/可执行规则）vs base**：胜率 33%，avg -0.2 vs 7.8——更差（但被 15% 超时污染）。
- **v6（禁开局酿酒厂/翻面才算数）+ 30s 超时 vs base**：胜率 67% vs 33%，但 avg 5.8 vs 7.5、负分 33% vs 17%——胜率升但 VP/负分更差，且 **v6 开局酿酒厂率 20% 与 base 相同（没修掉）**。结论：单局方差太大，6 局 A/B 难以归因。
- **模型能做的正确模式**（61 VP 局总结）：连商人位（£3 运河）→ 建煤/制造/棉纺 → 卖出翻面兑现 → 现金不足贷款补 → 重复。这是值得写进 prompt 的"好路径"。
