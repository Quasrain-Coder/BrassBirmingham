...
function actionKind(desc) {
    if (desc.startsWith('在'))
        return 'build';
    if (desc.startsWith('铺'))
        return 'network';
    if (desc.startsWith('卖出'))
        return 'sell';
    if (desc.startsWith('研发'))
        return 'develop';
    if (desc.startsWith('贷款'))
        return 'loan';
    if (desc.startsWith('搜寻'))
        return 'scout';
    if (desc.startsWith('跳过'))
        return 'pass';
    return 'unknown';
}
function hist(nums) {
...
function main() {
    ...
    const gameKey = (g) => `${g.seed}:${g.mirrored}`;
    const lostSeats = new Map();
    for (const g of games) {
        const lost = new Set();
        g.seatLabels.forEach((label, seat) => {
            const lost_ = g.winner === null ? false : g.winner !== seat;
            if (lost_)
                lost.add(seat);
        });
    ...
    for (const g of games)
        labelOf.set(gameKey(g), g.seatLabels);
    ...
    for (const [label, ds] of byLabel) {
        ...
        console.log(`行动类型: ${[...kinds].sort((a, b) => b[1] - a[1]).map(([k, n]) => `${k}:${((n / ds.length) * 100).toFixed(0)}%`).join(' ')}`);
        ...
        // 输局早期决策抽样（运河前 3 轮）
        const earlyLost = ds.filter((d) => d.era === 'canal' &&
            d.round <= 3 &&
        ...
        if (earlyLost.length > 0) {
            ...
            for (const d of earlyLost.slice(0, 8)) {
                console.log(`  seed${d.seed}${d.mirrored ? '(镜)' : ''} 轮${d.round} rank${d.chosenRank} ` +
