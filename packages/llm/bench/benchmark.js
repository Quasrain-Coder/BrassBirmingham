...
async function playOne(players, seed) {
    const agents = SPEC
    ...
    while (state.phase !== 'game-over') {
        ...
        const a = SPEC
            ...
            : agents[player].chooseAction(state, legal);
...
async function stats(players) {
    ...
    for (let g = 0; g < GAMES; g++) {
        ...
        if ((g + 1) % 10 === 0) {
            ...
            console.log(`[${players}p ${g + 1}/${GAMES}] 累计人均 ${avg.toFixed(1)} 耗时 ${((Date.now() - t0) / 1000).toFixed(0)}s`);
        }
    }
    const flat = all.flat();
    ...
    console.log(`${players}p × ${GAMES}: 人均 ${avg.toFixed(1)} | 胜者均 ${wAvg.toFixed(1)} | 胜者最高 ${wMax} | 单人最高 ${pMax} | ${((Date.now() - t0) / 1000).toFixed(0)}s`);
}
await stats(4);
await stats(2);
