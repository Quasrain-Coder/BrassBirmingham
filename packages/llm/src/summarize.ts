...
function describeLoan(state: GameState, player: PlayerIndex): string {
  ...
  const warning =
    after < 0
      ? `【危险：贷后收入${after}为负，每轮将扣钱/拆板块/扣VP，禁止】`
      : cur < 0
        ? `【已在负收入，再贷更糟，禁止】`
        : '';
...
export const SYSTEM_PROMPT = [
  ...
  '- 贷款是开局标准动作不是禁区：启动现金 (£17) 撑不起酒厂/铁厂连建，运河时代前 4 轮内贷 1-2 次款是强打法的标志——只要翻面路径明确（酒厂 +5 / 铁厂 +3 / 煤矿 +4 收入），即使暂入负收入也会立刻转正回本。终局（收入已无需经营）贷款 ≈ 免费 £30。真正要避免的只有一种：收入已为负且没有当回合转正手段时再贷。',
