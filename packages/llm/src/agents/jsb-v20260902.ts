 ...
 * （0901：110.2）。结构选型调研（brass-assistant ISMCTS/NN-MCTS 均不及
 * 自家 2-ply 启发式的实测）与迭代全史见
...
const plugin: AgentPlugin = {
  ...
  create: () => {
    // 调参注入（bench 扫描用，生产不设）：BRASS_TUNE3_FLIP='{"sellWindowFull":8}'
    // 在 create 时读取（同进程多配置扫描：每局重新 merge）。
    const tuneEnv = process.env['BRASS_TUNE3_FLIP'];
    if (tuneEnv) Object.assign(CFG.flip, JSON.parse(tuneEnv));
    // 全域调参（消融用）：BRASS_TUNE='{"network":{...},"sell":{...},"era":{"railLate":{...}}}'
    const tuneAll = process.env['BRASS_TUNE3'];
    if (tuneAll) {
    ...
    return {
      decide: ({ state, seat, legal }) => {
        if (legal.length === 0) throw new Error('jsb-v20260902: no legal actions');
