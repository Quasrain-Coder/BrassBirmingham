...
const plugin: AgentPlugin = {
  ...
  create: () => {
    // 调参注入（bench 扫描用，生产不设）：BRASS_TUNE2_FLIP='{"sellWindowFull":8}'
    // 在 create 时读取（同进程多配置扫描：每局重新 merge）。
    const tuneEnv = process.env['BRASS_TUNE2_FLIP'];
    if (tuneEnv) Object.assign(CFG.flip, JSON.parse(tuneEnv));
    // 全域调参（消融用）：BRASS_TUNE='{"network":{...},"sell":{...},"era":{"railLate":{...}}}'
    const tuneAll = process.env['BRASS_TUNE2'];
    if (tuneAll) {
    ...
    return {
      decide: ({ state, seat, legal }) => {
        if (legal.length === 0) throw new Error('jsb-v20260901: no legal actions');
