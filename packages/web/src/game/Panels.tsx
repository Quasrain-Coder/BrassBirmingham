...
export function PlayerBoard({
  ...
  if (compact) {
    ...
    // 按时代内当前轮号 roundNow 匹配(引擎 state.round 跨时代不重置,rail 时代
    // 是 9~16,直接匹配会永远落空;缺省回退 state.round)
    const acts = (rounds.find((r) => r.round === (roundNow ?? state.round))?.actions ?? []).filter(
