...
function parseArgs(argv) {
    const args = { room: undefined, game: undefined, to: 'last-canal-round', db: './brass.db', noAi: false };
...
function main() {
    ...
    if (args.game !== undefined) {
        game = db.prepare('SELECT * FROM games WHERE id = ?').get(args.game);
    }
    else if (args.room !== undefined) {
        game = db.prepare('SELECT * FROM games WHERE room_code = ?').get(args.room);
    }
    else {
        ...
        if (playing.length === 1)
            ...
            throw new Error(`库中有 ${playing.length} 个进行中对局,请用 --room/--game 指定`);
    }
    if (game === undefined)
        throw new Error('找不到对局');
    console.log(`game: ${game.id} (room=${game.room_code}, ${game.player_count}P, seed=${game.seed}, status=${game.status})`);
    if (game.status !== 'playing')
        throw new Error(`对局状态 ${game.status},仅支持 playing`);
    // 2) 重放已有行动
    let state = newGame(game.player_count, game.seed);
    const recs = [];
    const record = (s, player, action) => {
    ...
    // 4) 计算截断点
    let cutIdx;
    const lastRoundOf = (era) => {
        const eraRecs = recs.filter((r) => r.era === era);
        if (eraRecs.length === 0)
            throw new Error(`模拟中没有 ${era} 时代行动?`);
    ...
    if (args.to === 'last-canal-round' || args.to === 'last-rail-round') {
        ...
        if (!Number.isInteger(n) || n < 1)
            throw new Error(`--to ${args.to}: N 须为正整数`);
        cutIdx = Math.min(existing.length + n, recs.length);
    }
    else {
        const m = /^(canal|rail):(\d+)$/.exec(args.to);
        if (m === null)
            throw new Error(`--to ${args.to}: 支持 last-canal-round | last-rail-round | canal:N | rail:N | +N`);
        ...
        if (targetRound === undefined)
            throw new Error(`--to ${args.to}: 模拟未到达该状态`);
    ...
    // 6) 校验:全新重放写库后的行动序列
    let chk = newGame(game.player_count, game.seed);
    const rows = db
        .prepare('SELECT player, action FROM actions WHERE game_id = ? ORDER BY seq')
        .all(game.id);
    for (const row of rows) {
        chk = applyAction(chk, JSON.parse(row.action));
    }
    const cur = chk.turnOrder[chk.currentPlayerIdx];
    console.log(`verify: era=${chk.era} round=${chk.round} phase=${chk.phase} currentSeat=${cur} ` +
