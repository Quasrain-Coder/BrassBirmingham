...
export type ServerMessage =
  ...
  | { type: 'snapshot'; protocolVersion: number; seq: number; state: FilteredState; legalActions: Action[]; turnHold?: PlayerIndex | null; playedCards?: Card[][]; eraActions?: { action: Action; moneyDelta: number; note?: 'round-income' }[][] } // turnHold：该座位行动完但尚未显式"结束回合"——对局被扣住,等其 end_turn/reset_turn；playedCards：各座位本时代已打出的牌(按打出顺序,Wild 不入列)；eraActions：各座位本时代全部行动(按顺序)及实际现金变化
