...
5. **允许使用原版游戏素材**（美术、卡牌文字等）——本项目为个人非商用游玩（见 README Legal note）；无现成素材可用的部分再自绘简化 SVG 兜底。**素材不进 git**：由 `npm run fetch-assets -w @brass/web` 本地生成到 `packages/web/public/assets/`（已 gitignore，来源见 `packages/web/scripts/asset-manifest.json`）。
