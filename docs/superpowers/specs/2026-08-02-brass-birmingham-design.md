...
2. 组装 prompt：结构化文字局势摘要（非 JSON dump：资金/收入/手牌、各人产业与网络、市场价格与存量、时代与轮次）+ 候选行动列表（每个附一句话解释）。system 为完全静态前缀（规则要点 + 策略要点 + 经验法则——后者由内置启发式 jsb-v20260902b 自对局 100 局复盘提炼，见 `packages/llm/bench/docs/2026-09-03-jsb0902b-lessons.md`），吃 prompt 缓存；动态内容一律放 user
