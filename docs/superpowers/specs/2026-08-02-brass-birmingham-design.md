...
**行动枚举的规范化（canonicalization）**：资源来源选择（哪块煤、哪桶啤酒）默认按"最便宜/最短路"规则自动解析，不为等价来源枚举多个行动——只有来源选择产生实质差异（如触发不同商人奖励、耗干对手酿酒厂翻面）时才拆分为不同行动。否则 sell 链路 × 煤来源 × 啤酒来源的组合爆炸会拖垮 web 高亮和 LLM 预筛。
